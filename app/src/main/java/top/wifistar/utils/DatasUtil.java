...
public class DatasUtil {
	...
	public static final User curUser = new User("自己", HEADIMG[0]);
	static {
		User user1 = new User("张三", HEADIMG[1]);
		User user2 = new User("李四", HEADIMG[2]);
		User user3 = new User("隔壁老王", HEADIMG[3]);
		User user4 = new User("赵六", HEADIMG[4]);
		User user5 = new User("田七", HEADIMG[5]);
		User user6 = new User("Naoki", HEADIMG[6]);
		User user7 = new User("这个名字是不是很长，哈哈！因为我是用来测试换行的", HEADIMG[7]);
	...
	public static List<Moment> createCircleDatas(Moment moment) {
		List<Moment> circleDatas = new ArrayList<Moment>();
		for (int i = 0; i < 15; i++) {
			User user = getUser();
			moment.setObjectId(String.valueOf(circleId++));
			moment.setUser(user);
			moment.setContent(getContent());

			//moment.setFavors(createFavortItemList(moment));
			moment.setComments(createCommentItemList());
			int type = getRandomNum(10) % 2;
			if (type == 0) {
				moment.setType("1");// 链接
				moment.setLinkImg("http://pics.sc.chinaz.com/Files/pic/icons128/2264/%E8%85%BE%E8%AE%AFQQ%E5%9B%BE%E6%A0%87%E4%B8%8B%E8%BD%BD1.png");
				...
				moment.setVideoUrl(videoUrl);
                moment.setVideoImgUrl(videoImgUrl);
			}
			circleDatas.add(moment);
		}
	...
	public static List<User> createFavortItemList(Moment moment) {
		BmobRelation relation = moment.getFavors();
		int size = getRandomNum(users.size());
		List<User> items = new ArrayList<User>();
//		List<String> history = new ArrayList<String>();
//		if (size > 0) {
//			for (int i = 0; i < size; i++) {
//				User newItem = createFavortItem();
...
//		}
	...
	public static Comment createComment() {
		...
		item.setMomentId(String.valueOf(commentId++));
		item.setContent("哈哈");
		User user = getUser();
		item.setUser(user);
		if (getRandomNum(10) % 2 == 0) {
			while (true) {
				...
				if (!user.getObjectId().equals(replyUser.getObjectId())) {
	 ...
	 * @return
	 */
	public static Comment createPublicComment(String content){
		Comment item = new Comment();
		item.setMomentId(String.valueOf(commentId++));
	 ...
	 * @return
	 */
	public static Comment createReplyComment(User replyUser, String content){
		Comment item = new Comment();
		item.setMomentId(String.valueOf(commentId++));
		item.setContent(content);
		item.setUser(curUser);
	...
	public static Moment createVideoItem(String videoUrl, String imgUrl){
		...
		item.setObjectId(String.valueOf(circleId++));
		item.setUser(curUser);
		//item.setContent(getContent());

		//item.setFavors(createFavortItemList());
