...
public class BadgeView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    public static final int POSITION_CENTER_RIGHT = 16;
    public static final int POSITION_CENTER_LEFT = 17;
    public static final int POSITION_TEXT_RIGHT = 18;
    ...
    private Drawable getDefaultBackground() {
        ...
        if (!hasCircle) {
            ...
            drawable1 = context.getResources().getDrawable(R.drawable.red_point_with_circle);
        }

        return hasCircle ? drawable1 : drawable;
    }
    ...
    private void applyLayoutParams() {
        ...
        switch (badgePosition) {
            ...
            case POSITION_TEXT_RIGHT:
                lp.gravity = Gravity.LEFT | Gravity.CENTER;

                if (target instanceof TextView) {
                    TextView tvTarget = (TextView) target;
                    float textPaintWidth = tvTarget.getPaint().measureText(tvTarget.getText().toString());
                    textPaintWidth += textPaintWidth / 3;

                    int drawableWidth = 0;

                    Drawable[] drawables = tvTarget.getCompoundDrawables();
                    Drawable drawableLeft = drawables[0];
                    if (drawableLeft != null) {
                        drawableWidth = drawableLeft.getIntrinsicWidth() + 2 * getCompoundDrawablePadding();
                    }

                    lp.setMargins(tvTarget.getPaddingLeft() + badgeMarginH + drawableWidth + (int) textPaintWidth, 0, 0, 0);
                }
                break;
            default:
                break;
