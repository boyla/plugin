...
public class TopReminder extends RelativeLayout {
    ...
    public static final int THEME_SUCCESS = 3;
    private int iconRes;
    ...
    private void updateTheme() {
        if (reminderTheme == THEME_DEFAULT) {
            ...
            content.setBackgroundResource(R.color.block_tip_bg);
            if (iconRes > 0) {
                icon.setImageResource(iconRes);
            } else {
                icon.setImageResource(R.drawable.notification_icon_network);
            }
        } else if (reminderTheme == THEME_SUCCESS) {
            content.setBackgroundResource(R.color.success_theme_background_color);
    ...
    public void show(CharSequence text, boolean swipeToDismiss, boolean showIcon) {
